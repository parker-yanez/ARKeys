...
for _ in range(5):
    ...
    wpm_str = f'{wpm:3d}'
    # center WPM text
    w_x = (width - draw.textsize(wpm_str, font=font_wpm)[0]) // 2
    draw.text((w_x, 35), wpm_str, font=font_wpm, fill=0)

    # 5) Draw Accuracy
    acc_str = f'{acc:.1f}%'
    a_x = (width - draw.textsize(acc_str, font=font_acc)[0]) // 2
    ...
    # 7) Draw Footer (time/status)
    footer_str = f'{now}  {status}'
    draw.text((5, 235), footer_str, font=font_footer, fill=0)
